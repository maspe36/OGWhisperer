...
public:
	...
	int _Statis = 1;
	// Number of attacks a creature has left
	int _Flurry = 1;
	int _OriginalFlurry = 1;
	bool CanAttack;

	Creature(vector<int> Cost, string Name, string FlavourText, string EffectText, _Color Color, _CardType CardType, vector<_EffectType> Effects, int OriginalAttack, int OriginalDefense);

	void PreAttackMaintenance();
	void Attacking(Creature* DefendingCreature);
