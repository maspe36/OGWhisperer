...
Tavern_Hearth::Tavern_Hearth() 
	: Constant({ 0,0,3,0,0,0 }, "Tavern Hearth", "All red creatures have +1/+0.", "When theres smoke theres fire!", _Color::Fire, _CardType::_Constant, { _EffectType::Introduce })
{
	_AttackBuff = 1;
...
void Tavern_Hearth::Effect()
{
	...
	for (auto i : Owner->CurrentGame->CardOrder) 
	{
		...
		if (i->CardType == Card::_CardType::_Creature && i->Color == Card::_Color::Fire) 
		{
			...
			if (!IsAlreadyBuffed)
			{
				Creature* CreatureCard = dynamic_cast<Creature*>(i);
				CreatureBuffNoTrigger({ CreatureCard }, Owner);
			}
		}
	}
}
...
bool Tavern_Hearth::IsEffectTriggered(Action* CurrentAction)
{
	...
	{
		// Check all cards being summoned, if any were a creature, red effect triggers
		for (auto i : CurrentAction->CardTargets)
		{
			...
			else if (i->CardType == Card::_CardType::_Creature && i->Color == Card::_Color::Fire) 
			{
				return true;
			}
		}
