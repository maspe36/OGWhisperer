...
/* Returns a formatted JSON of all the creatures in play for this player.
Cannot be returned alone, it is missing a wrapping {} on purpose.*/
string Player::CreaturesInPlayToString()
{
	ostringstream CreaturesInPlayString;

	for (auto i : CreaturesInPlay) {
		CreaturesInPlayString << (i->ToString()) << "; ";
	}

	return CreaturesInPlayString.str();
}
